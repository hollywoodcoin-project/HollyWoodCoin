...
{
    return GetBoolArg("-testnet", false) ? 21030 : 11030;
}
...
Value stop(const Array &params, bool fHelp)
{
    ...
    if(fHelp || params.size() > 1)
    {
        throw runtime_error("stop\n""Stop Universe server.");
    }

    // Shutdown will take long enough that the response should get back
    StartShutdown();

    return "Universe server stopping";
}
...
string HTTPPost(const string &strMsg, const map<string, string> &mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: universe-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
bool ClientAllowed(const boost::asio::ip::address &address)
{
    ...
    if(address == asio::ip::address_v4::loopback() || address == asio::ip::address_v6::loopback() || (address.is_v4()
...
template<typename Protocol>
static void RPCAcceptHandler(
//	boost::shared_ptr<basic_socket_acceptor<Protocol, SocketAcceptorService> > acceptor,
    boost::shared_ptr<basic_socket_acceptor<Protocol> > acceptor,
    ssl::context &context,
    bool fUseSSL,
...
void ThreadRPCServer2(void *parg)
{
    ...
    if((mapArgs["-rpcpassword"] == "") || (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use universed";
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
                                             _("%s, you must set a rpcpassword in the configuration file:\n %s\n"
                                               ...
                                               "for example: alertnotify=echo %%s | mail -s \"Universe Alert\" admin@foo.com\n"),
                                             strWhatAmI.c_str(),
...
//	ssl::context context(io_service, ssl::context::sslv23);
    ...
    // ^^^
    // Hope that fixed, but who knows?
    asio::io_service io_service;
    ssl::context context(ssl::context_base::sslv23);
...
{
    Array params;
    BOOST_FOREACH(const std::string &param, strParams) { params.push_back(param); }
